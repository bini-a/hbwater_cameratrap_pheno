...
#### ImageFile Class
Within this script is a class named `ImageFile`, from which an object containing information from a specified filename is to be created.
...
The first image file in image_file_list is plotted, allowing the user to draw the first region of interest (see section 2.1 `RoiPoly` for instruction).
...
### Button Events
This section details instruction and function for each interactive button. These buttons are part of `matplotlib`'s `Button` module. Creating one requires an event function, as well as initialization as seen below for the `Confirm` button.
...
To create a new region of interest, the user must click restart masking. The user should then create a new region of interest and confirm to proceed to apply the new mask.

```{r image-restart-masking, echo = FALSE}
#knitr::include_graphics("path to screenshot") #screenshot restart button popping up
```

After creating a new ROI, the user has the option to confirm or to restart and draw a new ROI.
...
```{r next-prev-button, echo = FALSE}
#knittr::include_graphics("filepath to screenshot")
```
...
```{r finish_masking_button, echo = FALSE}
...
#knittr::include_graphics("path to screenshot")# screenshot of wateryear folder output
```
#### Example Output for Dataframes
```{r Dataframe-id-mask, echo = FALSE}
#knittr::include_graphics("path to screenshot")# screenshot of mask id to associated mask dataframe
```

```{r Dataframe-date-id, echo = FALSE}
#knittr::include_graphics("path to screenshot")# screenshot of date to mask id dataframe
```
