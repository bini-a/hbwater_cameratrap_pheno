...
running the script in *Google Colaboratory*. Before renaming images contained in a local directory, a few modifications to the script will need to be made; however, the same general principles will still apply. For instructions on running the script on files on your local computer, see **section 1.2**. 
...
information. There are many complementary methods in this script; for more information, see documentation within the script itself. 
...
the one housed in Google Colab. RUpdate the file path and run the main method to
rename your files.
