...
Because of `shutil` and `os`, this same method can also be used to copy local files without requiring any modifications beyond changing the file paths for the source and destination directories.
...
the images taken by the Bushnell field cameras used in our study. These will have to be manually
*adjusted* for images with time series information contained elsewhere. If the images
do not contain timestamps, the script must be modified to follow a separate file
naming convention in the `generate_picName` function.
